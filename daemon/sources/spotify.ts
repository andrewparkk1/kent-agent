...
function defaultGetCredentials(): SpotifyCreds | null {
  try {
...
export function createSpotifySource(
  ...
  return {
    ...
    if (config.accessToken !== undefined) {
      ...
      const creds =
        config.credentials !== undefined ? config.credentials : defaultGetCredentials();
      if (!creds) return [];
...
export const spotify: Source = createSpotifySource();
