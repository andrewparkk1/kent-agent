...
function ToolCallBlock({ content }: { content: string }) {
  ...
  // Try to extract a summary line from the tool output
  ...
  return (
    ...
    >
      ...
      >
        <Terminal size={12} />
        <span className="font-mono truncate max-w-[400px]">{firstLine}</span>
...
export function ChatPage({ threadId: initialThreadId, onThreadCreated }: {
  ...
  // Load existing thread messages when threadId changes from props
  useEffect(() => {
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {loadingHistory ? (
          ...
          <div className="max-w-[680px] mx-auto px-6 py-8 space-y-1">
            <AnimatePresence initial={false}>
              {messages.map((msg) => {
                // System messages = tool calls — render as collapsible block
                if (msg.role === "system") {
                  return <ToolCallBlock key={msg.id} content={msg.content} />;
                }
      ...
      {/* Input */}
      <div className={`w-full transition-all duration-300 ${isEmpty ? "pb-[30vh]" : "pb-6"}`}>
        <div className="max-w-[680px] mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: isEmpty ? 0.3 : 0, duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] }}
            className="relative bg-background border border-border/70 rounded-2xl shadow-[0_1px_6px_-1px_rgba(0,0,0,0.06)] hover:shadow-[0_2px_12px_-2px_rgba(0,0,0,0.08)] focus-within:shadow-[0_2px_12px_-2px_rgba(0,0,0,0.08)] focus-within:border-border/90 transition-all duration-300"
          >
            <textarea
              ref={inputRef}
